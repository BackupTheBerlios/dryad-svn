...
analyze::analyze(conf *c)
{
	...
	if( c == NULL )
	{
		dryerr(1, 1, "Invalid conf passed to analyze!\nAborting!\n");
		exit(1);
	}
	...
	for( int q = 0; q < c->num_daemons(); q++ )
	{
		...
		if(!rpt->dlptr)
		{
			dryerr(1, 3, "Failed to open ", c->daemon_get(c->daemon_name(q)->ascii(), "lib_handler"), "!\n");
			free(rpt);
			continue;
		}
		lt_dlerror();
		...
		if( (error = lt_dlerror()) )
		{
			dryerr(1, 3, "Failed to export symbol dryad_once!\n", error, "\n");
			free(rpt);
			continue;
		}
		...
		if( (error = lt_dlerror()) )
		{
			dryerr(1, 3, "Failed to export symbol dryad_many!\n", error, "\n");
			free(rpt);
			continue;
		}
		...
		if( (error = lt_dlerror()) )
		{
			dryerr(1, 3, "Failed to export symbol dryad_config!\n", error, "\n");
			free(rpt);
			continue;
		}
		(*cnf)(c);
	...
	if( t == NULL )
	{
		dryerr(1, 1, "A default lib_handler has not been specified!\nAborting!\n");
		exit(1);
	}
	def_rep->dlptr = lt_dlopen( t->ascii() );
	if( def_rep->dlptr == NULL )
	{
		dryerr(1, 5, "Failed to lt_dlopen() the default lib_handler (", t->ascii(), "):\n", lt_dlerror(), "\nAborting!\n");
		exit(1);
	}
	lt_dlerror();
	...
	if( (error = lt_dlerror()) )
	{
		dryerr(1, 5, "Failed to resolve symbol dryad_once in default lib_handler (", t->ascii(), "):\n",error, "\nAborting!\n");
		exit(1);
	}
...
struct severity *analyze::build_severity_struct(const char *daemon, char *level)
{
	...
	if( daemon == NULL )
	{
		...
		if( tmp == NULL )
		{
			dryerr(1, 3, "Failed to get ", build->ascii(), "!\nAborting!\n");
			exit(1);
		}
		delete build;
		if( ! atoi(tmp->ascii()) )
